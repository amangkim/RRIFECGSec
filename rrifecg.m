function [TrainedStruct] = rrifecg(varargin)
% Generating the RR-Interval Framed sliced regference signal by using ML (in time domain)
%
% Usage:
%	[Structred_Reference (SR)] = rrifecg(ECG_data, SampleFreq, TargetFrame, DisplayOption)
% Output: 
...
%   SR.TrainData:       [[slot ECG]; Nx2] Training data
...
%   TargetFrame:        Traget Number of Frame between RR-Interval
...
for i= 2:NumSlice    
    ...
    if qualidx>=0.8
        ...
        Dat4ML = [Dat4ML; [slot(:) ecg1]];  
    end
end

 ref_plot = mean(ecg1set,2);
 ref_mode = mode(ecg1set,2);
